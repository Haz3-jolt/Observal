...
class TestCidrSupport:
    ...
    def test_default_docker_cidr_trusts_nginx(self):
        ...
        from api.middleware.trusted_proxy import _is_trusted, _parse_trusted

        exact, networks = _parse_trusted("172.16.0.0/12,10.0.0.0/8,192.168.0.0/16,127.0.0.1")
        # Typical Docker-assigned nginx IPs
        assert _is_trusted("172.18.0.2", exact, networks)
        assert _is_trusted("172.20.0.5", exact, networks)
        assert _is_trusted("10.0.2.1", exact, networks)
        assert _is_trusted("127.0.0.1", exact, networks)
        # Public IPs should NOT be trusted
        assert not _is_trusted("8.8.8.8", exact, networks)
        assert not _is_trusted("203.0.113.1", exact, networks)
