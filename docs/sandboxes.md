...
`source_url` + `sandbox_path` can point at a Dockerfile or sandbox source tree, but Observal does not build or publish images from that path today. Publish a built image/artifact ref in `image`.
