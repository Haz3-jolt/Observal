...
async def upload_layer_snapshot(
    ...
    """
    try:
        result = await ch_query(check_sql, {
    ...
    optic.info(
        ...
        req.hash, len(req.files), total_size,
    )
...
async def get_layer_snapshot(
    ...
    return LayerSnapshotDetail(
        hash=row["hash"],
        ide=content.get("ide", row.get("ide", "")),
...
async def diff_layer_snapshots(
    ...
    files_a = {f["path"]: f for f in snapshots[hash_a].get("files", [])}
    files_b = {f["path"]: f for f in snapshots[hash_b].get("files", [])}
